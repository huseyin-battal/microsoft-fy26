...
**Bulut final deneyi — Quantinuum `h2-1e` emülatörü (gürültü modeli):**
- **Kota dersi:** 100 shot'lık iş **1076 eHQC** talep etti; tek seferlik (One-Time) kota 1000 (kalan 994) → `NotEnoughQuota`, iş çalışmadan reddedildi (harcama olmadı). Maliyet ≈ `5 + shots × (1q + 10×2q + 5×ölçüm)/5000`; bizim devrede shot başına ~10,7 eHQC.
- `shots=10` (~112 eHQC, ~%11 kota) ile çalıştı (~1 saat kuyruk). Öncesinde `h2-1sc` syntax checker 8 satırlık programı kabul etmişti (Succeeded; tasarım gereği tümü-0 çıktı) — yani her iki sağlayıcının derleme katmanı da küçük ölçekte geçildi.
- **Sonuç: düz histogram** — `{001:0.1, 011:0.2, 100:0.2, 111:0.2, 000:0.2, 110:0.1}`, beklenen `[1,1,1]` (indeks 7) tepesi yok.
- **Sebep (beklenen NISQ gerçeği):** `h2-1e` gerçekçi H2 gürültü modeli kullanır. 601 ccx ≈ ~5.000 iki-kübit kapı; ~%99,8 kapı sadakatiyle başarı ≈ 0,998^5000 ≈ 5×10⁻⁵ — sinyal tamamen gürültüde eriyor. Bu, §5'teki Grand Challenges gürültü tablosunun (4 kübitlik devrede bile p=0.01 → %46, p=0.05 → %15) deneysel doğrulaması: **QROM tabanlı Grover bu derinlikte NISQ donanımında pratik değil — teori kendi deneyimizle doğrulandı.**
...
**Nihai durum (bkz. Bölüm 13):** Mekanizma 64 satır × 2 sütun ölçeğinde yerel simülatörde uçtan uca doğrulandı — %98 olasılıkla doğru indeks, klasik kontrolle örtüşüyor. Bulut tarafında: Rigetti QVM servis hatası verdi (Bölüm 12); Quantinuum syntax checker 8 satırlık programı kabul etti; `h2-1e` gürültü-modelli emülatör ise düz histogram döndürdü — ~5.000 iki-kübit kapılık derinlik NISQ gürültüsünde sinyali tamamen eritiyor (Bölüm 13). Doğruluk ideal simülatörde kanıtlandı; NISQ pratiği-dışılığı da deneysel olarak doğrulandı. Bekleyen tek düzeltme: sütun bit sırası (endianness) — `reversed()` yaması ve palindrom-dışı regresyon testi.
